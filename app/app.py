...
@admin_required
def ai_health():
    """Check if the goldco-ai API is reachable"""
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({
                'status': 'online',
                'goldco_api': health_data
            })
        else:
            return jsonify({
                'status': 'error',
                'message': 'AI API returned non-200 status'
            }), 503
    except requests.exceptions.Timeout:
        return jsonify({
            'status': 'error',
            'message': 'AI API timeout - service may be starting up'
        }), 503
    except requests.exceptions.ConnectionError:
        return jsonify({
            'status': 'error',
            'message': 'Cannot connect to AI API - service may be down'
        }), 503
    except Exception as e:
        return jsonify({
            'status': 'error',
...
@admin_required
def ai_chat_api():
    """Proxy chat requests to the goldco-ai API"""
    try:
        data = request.get_json()
        question = data.get('question', '').strip()
        model = data.get('model', 'mistral:7b-instruct-q4_K_M')  # Default to smart model
        ...
        try:
            ...
            if ai_response.status_code == 200:
                ...
                answer = ai_data.get('answer', 'No answer received')
                ...
                try:
                    supabase.table('cms_activity_log').insert({
                        ...
                        'details': f'Asked AI: {question[:50]}...'
                    }).execute()
                except Exception as log_error:
                    print(f"Warning: Failed to log AI chat activity: {log_error}")
                ...
                return jsonify({
                    'error': f'AI API returned status {ai_response.status_code}',
            ...
            return jsonify({
                ...
                'message': 'The request took too long. This is normal for complex queries with the Smart model.'
            }), 504
        except requests.exceptions.ConnectionError:
            return jsonify({
                'error': 'Connection failed',
                'message': 'Cannot connect to AI service. Please contact your administrator.'
            }), 503
